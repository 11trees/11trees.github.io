...
| `technologyLabel` | string | 是 | 带中文说明的完整标签，如 `"SMR（叠瓦磁记录）"` |
| `technologyColor` | string | 是 | 前端展示颜色：`"red"`（SMR）/ `"green"`（PMR/CMR）/ `"purple"`（HAMR） |
...
| `gradeLabel` | string \| null | 否 | 定位标签文案，如 `"消费级硬盘"` / `"企业级硬盘"` |
...
以下展示逻辑由前端根据 `technology` 字段自行处理，**不需要后端返回**：

### 兼容性建议

| 技术类型 | NAS | RAID 重建 | ZFS / 数据库 |
|---|---|---|---|
| PMR / CMR | 推荐 | 推荐 | 推荐 |
| SMR | 不推荐 | 不推荐 | 不推荐 |
| HAMR | 不推荐 | 不推荐 | 不推荐 |
...
| 技术类型 | 警告文案 |
|---|---|
| SMR | SMR 硬盘在持续写入场景下性能可能下降，不适合频繁写入或多盘位 RAID 使用。 |
| PMR / CMR / HAMR | 无 |
